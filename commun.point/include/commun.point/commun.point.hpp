...
struct structures {
    ...
    /**
        \brief Structure containing information about the safe in the points balance, to which it's attached
        \ingroup safe_t
    */
    struct safe_t {
        int64_t  unlocked;   //!< Amount of unlocked points in the safe, share_type
    ...
    */
    ...
    struct account_struct {
        ...
        eosio::binary_extension<uint32_t, write_strategy::no_value> version; // if has value, it must be 0. (can be flags to indicate enabled addons; will require custom `unpack`)
