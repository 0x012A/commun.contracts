...
struct commun_list_api: base_contract_api {
    ...
    action_result sys_ban(name client, name locked, std::string reason) {
        return push_msig(N(ban), {{_code, cfg::client_permission_name}}, {client}, args()
            ("commun_code", symbol_code())
            ("account", locked)
            ("reason", reason)
        );
    }

    action_result sys_unban(name client, name locked, std::string reason) {
        return push_msig(N(unban), {{_code, cfg::client_permission_name}}, {client}, args()
            ("commun_code", symbol_code())
            ("account", locked)
            ("reason", reason)
        );
    ...
    action_result ban(symbol_code commun_code, name locker, name locked, std::string reason) {
        return push(N(ban), locker, args()
            ("commun_code", commun_code)
            ("account", locked)
            ("reason", reason)
        );
    ...
    action_result unban(symbol_code commun_code, name locker, name locked, std::string reason) {
        return push(N(unban), locker, args()
            ("commun_code", commun_code)
            ("account", locked)
            ("reason", reason)
        );
