...
public:
        ...
        , community({this, cfg::list_name})
    {
        ...
        link_authority(cfg::list_name, cfg::list_name, cfg::client_permission_name, N(ban));
        link_authority(cfg::list_name, cfg::list_name, cfg::client_permission_name, N(unban));
    }
...
BOOST_FIXTURE_TEST_CASE(ban_test, commun_list_tester) try {
    BOOST_TEST_MESSAGE("ban test");
    BOOST_TEST_MESSAGE("-- success ban by application");
    BOOST_CHECK_EQUAL(success(), community.sys_ban(_client, _bob, "ban"));
    BOOST_TEST_MESSAGE("-- success unban by application");
    BOOST_CHECK_EQUAL(success(), community.sys_unban(_client, _bob, "unban"));
    BOOST_TEST_MESSAGE("-- fail with no POINT");
    BOOST_CHECK_EQUAL(err.no_point_symbol, community.ban(_token_code, _golos, _bob, "ban"));
    BOOST_CHECK_EQUAL(err.no_point_symbol, community.unban(_token_code, _golos, _bob, "unban"));
    create_token(_golos, _token);
    BOOST_TEST_MESSAGE("-- fail with no community");
    BOOST_CHECK_EQUAL(err.no_community, community.ban(_token_code, _golos, _bob, "ban"));
    BOOST_CHECK_EQUAL(err.no_community, community.unban(_token_code, _golos, _bob, "unban"));
    BOOST_CHECK_EQUAL(success(), community.create(cfg::list_name, _token_code, "community_name"));
    BOOST_TEST_MESSAGE("-- fail with wrong authority");
    BOOST_CHECK_EQUAL(err.missing_auth(_golos), community.ban(_token_code, _alice, _bob, "ban"));
    BOOST_CHECK_EQUAL(err.missing_auth(_golos), community.unban(_token_code, _alice, _bob, "unban"));
    BOOST_TEST_MESSAGE("-- success ban by community");
    BOOST_CHECK_EQUAL(success(), community.ban(_token_code, _golos, _bob, "ban"));
    BOOST_TEST_MESSAGE("-- success unban by community");
    BOOST_CHECK_EQUAL(success(), community.unban(_token_code, _golos, _bob, "unban"));
} FC_LOG_AND_RETHROW()
