...
public:
    ...
    struct errors: contract_error_messages {
        const string no_point_account = amsg("no point account object found");
...
BOOST_FIXTURE_TEST_CASE(no_balance, commun_point_safe_tester) try {
    ...
    BOOST_CHECK_EQUAL(err.no_point_account, point.enable_safe(_bob, 0, _delay));
    BOOST_CHECK_EQUAL(err.no_point_account, point.disable_safe(_bob, mod_id));
    BOOST_CHECK_EQUAL(err.no_point_account, point.unlock_safe(_bob, mod_id, 1));
    BOOST_CHECK_EQUAL(err.no_point_account, point.lock_safe(_bob, 1));
    BOOST_CHECK_EQUAL(err.no_point_account, point.modify_safe(_bob, mod_id, cfg::safe_max_delay/2));
