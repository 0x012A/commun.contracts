...
void point::close(name owner, symbol_code commun_code) {
    ...
    check(!it->has_safe(), "Cannot close because safe enabled.");
    it->validate();
...
void point::enablesafe(name owner, asset unlock, uint32_t delay, name trusted) {
    ...
    const auto& acc = tbl.get(scode.raw(), "no point account object found");
    ...
    tbl.modify(acc, owner, [&](auto& a) {
        a.create_safe(structures::safe_t{unlock.amount, delay, trusted});
    });
}
...
void point::applysafemod(name owner, name mod_id) {
    ...
    accounts tbl(_self, owner.value);
    const auto& acc = tbl.get(mod.commun_code.raw(), "no point account object found");
